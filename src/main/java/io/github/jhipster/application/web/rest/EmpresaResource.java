...
/**
 * REST controller for managing {@link io.github.jhipster.application.domain.Empresa}.
 */
@RestController
@RequestMapping("/api")
public class EmpresaResource {
     ...
     */
    @GetMapping("/empresas")
    public ResponseEntity<List<Empresa>> getAllEmpresas(EmpresaCriteria criteria, Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder) {
    ...
    */
    ...
    public ResponseEntity<List<Empresa>> searchEmpresas(@RequestParam String query, Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder) {
        log.debug("REST request to search for a page of Empresas for query {}", query);
        Page<Empresa> page = empresaService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(uriBuilder.queryParams(queryParams), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
