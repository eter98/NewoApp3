...
/**
 * REST controller for managing {@link io.github.jhipster.application.domain.Blog}.
 */
@RestController
@RequestMapping("/api")
public class BlogResource {
     ...
     */
    @GetMapping("/blogs")
    public ResponseEntity<List<Blog>> getAllBlogs(BlogCriteria criteria, Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder) {
    ...
    */
    ...
    public ResponseEntity<List<Blog>> searchBlogs(@RequestParam String query, Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder) {
        log.debug("REST request to search for a page of Blogs for query {}", query);
        Page<Blog> page = blogService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(uriBuilder.queryParams(queryParams), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
